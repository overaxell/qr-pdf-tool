...
import pandas as pd  # если понадобится дальше
import fitz  # PyMuPDF
import io
import zipfile
import requests
...
# --- ПОИСК БЕЛЫХ КВАДРАТОВ НА МАКЕТЕ ---
def detect_white_rectangles_in_pdf(
    ...
    """
    Ищет крупные почти белые прямоугольные области на первой странице PDF.
    ...
    for cnt in contours:
        ...
        # квадратоподобные области
        aspect = w / h if h != 0 else 0
        if aspect < 0.8 or aspect > 1.25:
            continue

        x_pt = x * page_w_pt / img_w
        y_pt = y * page_h_pt / img_h
                ...
                if qr_bytes:
                    with fitz.open(stream=pdf_bytes, filetype="pdf") as doc:
                        ...
                        if mode == "white_rect" and white_rects:
                            rx, ry, rw, rh = white_rects[0]  # берём самый крупный белый квадрат

                            margin_pt = mm_to_pt(1.0)
                            ...
                            if qr_size_pt <= 0:
                                # если квадрат слишком маленький, fallback в центр
                                qr_size_pt = page_w / 3
                                x_pt = (page_w - qr_size_pt) / 2
                                y_pt = (page_h - qr_size_pt) / 2
                            else:
                                # выравниваем QR по центру внутренней области
